...
public:
    ...
    {
        RCLCPP_INFO(this->get_logger(), "exer02_server_node_cpp 节点已启动（C++）!");
        // 3-1.创建一个订阅方（原生乌龟位姿 /turtle1/pose）
        sub_ = this->create_subscription<turtlesim::msg::Pose>("/turtle1/pose",10,std::bind(&Exer02Server::pose_cb,this,_1));
        // 3-2.创建一个服务端
        server_ = this->create_service<base_interfaces_demo::srv::Distance>("distance",std::bind(&Exer02Server::distance_cb,this,_1,_2));
        // 3-3.回调函数需要解析客户端数据并响应结果到客户端
    }
...
private:
    ...
    void distance_cb(base_interfaces_demo::srv::Distance_Request::SharedPtr request,
